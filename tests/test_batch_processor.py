...
def batch_processor(mock_snapshot_storage):
    return BatchProcessor(
        storage_backend=mock_snapshot_storage, batch_size=3, max_wait_time=None
    )
...
def test_add_item(batch_processor):
    ...
    """
    batch_processor.add_item("item1", input_value="input1")
...
def test_different_batch_sizes(mock_snapshot_storage):
    ...
    """
    processor_small = BatchProcessor(
        storage_backend=mock_snapshot_storage, batch_size=2
    )
    processor_large = BatchProcessor(
        storage_backend=mock_snapshot_storage, batch_size=5
    )
    ...
    # Large batch size
    processor_large.add_item("item1", input_value="input1")
    processor_large.add_item("item2", input_value="input2")
    mock_snapshot_storage.store_snapshot.assert_called_once()
...
def test_wait_time_functionality(mock_snapshot_storage):
    ...
    processor = BatchProcessor(
        storage_backend=mock_snapshot_storage, batch_size=10, max_wait_time=1
    )

    processor.add_item("item1", input_value="input1")
    assert mock_snapshot_storage.store_snapshot.call_count == 0
    time.sleep(1.5)  # Wait for the batch to be processed due to timeout

    processor.add_item(
        "item2", input_value="input2"
    )  # Trigger the check for wait time exceeded
    processor.shutdown()
    mock_snapshot_storage.store_snapshot.assert_called_once()
...
def test_wait_time_not_exceeded():
    ...
    processor = BatchProcessor(
        storage_backend=mock_storage, batch_size=10, max_wait_time=1
    )
